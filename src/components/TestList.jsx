  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-purple-50">
      ...
      <aside className="w-full md:w-64 bg-white border-r border-blue-200 p-6 flex-shrink-0">
        ...
        {/* Category List */}
        <div className="mb-8">
          <h4 className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-3">Categories</h4>
            ...
            {CATEGORIES.map(cat => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeCategory === cat.id
                  ? 'bg-purple-50 text-purple-700 font-bold border border-blue-100 shadow-sm'
        ...
        <div>
          <h4 className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-3">Exams</h4>
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-blue-800">
              ...
              {activeExam !== 'All' && <span className="text-blue-400 font-light"> / {activeExam}</span>}
            </h1>
            <p className="text-blue-500 mt-1">Found {tests.length} tests available for you.</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
            {tests.map((test) => (
              <div
                key={test._id}
                className="bg-white shadow-sm hover:shadow-xl rounded-2xl p-6 border border-blue-100 transition-all group relative overflow-hidden"
              >
                <div className="absolute top-0 left-0 w-1 h-full bg-blue-500 transform -tranblue-x-full group-hover:tranblue-x-0 transition-transform duration-300" />
                ...
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="text-blue-400 text-xs font-semibold">{test.type || 'Full Test'}</span>
                </div>

                <h3 className="text-lg font-bold text-blue-800 mb-2 group-hover:text-blue-700 transition-colors line-clamp-1">{test.name}</h3>
                ...
                <div className="flex items-center justify-between border-t border-blue-50 pt-4 mt-auto">
                  ...
                  <Link
                    ...
                    className="bg-blue-900 hover:bg-purple-900 text-white font-bold py-2 px-6 rounded-lg transition-all text-sm transform active:scale-95"
                  >
                    Start Test
                  </Link>
                </div>
          ...
          <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-blue-200">
            ...
            <p className="text-blue-500 text-lg font-medium">No tests found in this category.</p>
            <button
